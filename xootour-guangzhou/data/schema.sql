-- ============================================================
-- XOOTOUR Guangzhou Database Schema
-- 景点系统 + 5大品类门店系统（美食/酒店/景点/购物/娱乐）
-- ============================================================

CREATE DATABASE IF NOT EXISTS `xootour_guangzhou` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE `xootour_guangzhou`;
...
CREATE TABLE `tour_spot` (
    ...
    `city_id` INT NOT NULL DEFAULT 3 COMMENT '城市ID(广州=3)',
...
CREATE TABLE `tour_spot_route` (
    ...
    `duration_hours` DECIMAL(4,1) COMMENT '预计游玩时长(小时)',
    `route_nodes` JSON NOT NULL COMMENT '路线节点JSON数组',
    `description` TEXT COMMENT '路线描述与避坑指南',
    CONSTRAINT `fk_route_spot` FOREIGN KEY (`spot_id`) REFERENCES `tour_spot`(`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='景点推荐路线';
...
CREATE TABLE `store_category` (
    `id` INT PRIMARY KEY AUTO_INCREMENT,
    `name_zh` VARCHAR(50) NOT NULL COMMENT '中文分类名',
    `name_en` VARCHAR(50) NOT NULL COMMENT '英文分类名',
    `icon` VARCHAR(200) COMMENT '图标URL',
    `sort_order` INT DEFAULT 0 COMMENT '排序',
    `description` VARCHAR(500) COMMENT '分类说明',
    `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='门店分类表';

INSERT INTO `store_category` (`id`, `name_zh`, `name_en`, `sort_order`, `description`) VALUES
(1, '美食', 'Food', 1, '广州美食门店，覆盖粤菜/早茶/烧腊/肠粉/糖水/老字号及各地菜系'),
(2, '酒店', 'Hotel', 2, '广州住宿服务，覆盖星级酒店/精品民宿/江景酒店/温泉度假村/青年旅舍'),
(3, '景点', 'Attraction', 3, '广州旅游景点，覆盖5A/4A景区/博物馆/公园/寺庙/古镇/主题乐园'),
(4, '购物', 'Shopping', 4, '广州购物场所，覆盖商圈百货/特色街区/免税店/批发市场/文创市集'),
(5, '娱乐', 'Entertainment', 5, '广州娱乐场所，覆盖酒吧/KTV/密室/剧本杀/演出/粤剧/夜店');
...
CREATE TABLE `store` (
    ...
    `cover_img` VARCHAR(500) COMMENT '封面图URL',
    `seo_desc` VARCHAR(500) COMMENT 'SEO描述',
    `price_range` VARCHAR(50) COMMENT '价格档次',
    `rating` DECIMAL(2,1) DEFAULT 0.0 COMMENT '综合评分',
    `review_count` INT DEFAULT 0 COMMENT '点评数',
    `ranking_desc` VARCHAR(200) COMMENT '排名描述',
    `cuisine_tags` JSON COMMENT '标签JSON',
    `michelin_status` TINYINT DEFAULT 0 COMMENT '米其林状态(0=无,1=一星,2=二星,3=三星,4=必比登)',
...
CREATE TABLE `store_info` (
    ...
    `category_id` INT NOT NULL COMMENT '分类ID',
    `amap_id` VARCHAR(50) COMMENT '高德POI ID',
    `district` VARCHAR(100) COMMENT '行政区',
    `address_zh` TEXT COMMENT '中文详细地址',
    `address_en` TEXT COMMENT '英文详细地址',
    `lat` DECIMAL(10,7) COMMENT '纬度',
    ...
    `summary_zh` TEXT COMMENT '中文详情',
    `summary_en` TEXT COMMENT '英文详情',
    `visit_notice` TEXT COMMENT '消费须知',
    `signature_items` JSON COMMENT '招牌项目JSON',
    `tags` JSON COMMENT '标签JSON',
    `category_specific_fields` JSON COMMENT '品类专属字段JSON',
    `subratings` JSON COMMENT '分项评分JSON',
    `features` JSON COMMENT '特色/设施JSON',
    ...
    `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    UNIQUE KEY `uk_store_info_store` (`store_id`),
    CONSTRAINT `fk_store_info_store` FOREIGN KEY (`store_id`) REFERENCES `store`(`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='门店详情表';
