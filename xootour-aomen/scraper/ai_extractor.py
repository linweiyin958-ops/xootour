...
SYSTEM_PROMPT = """你是一名专业的澳门旅游景点数据结构化提取专家。你的任务是从多源原始数据（马蜂窝游记、携程景点信息、TripAdvisor英文点评、高德地图坐标）中，按照 XOOTOUR 系统的 L1~L5 分层结构，提取并结构化景点数据。
...

{
  ...
  "sub_tables": {
    ...
    "tour_spot_package": [
      {"package_name": "套餐名称", "price": 0.00, "description": "套餐说明", "currency": "MOP"}
    ]
  },
  "L5_quality_control": {
...
SPOT_SUPPLEMENT_PROMPT = """你是一名专业的澳门旅游景点数据补充专家。已有部分字段通过规则提取完成，现在需要你补充AI生成的字段。
...
STORE_SUPPLEMENT_PROMPT = """你是一名专业的澳门旅游服务门店数据补充专家。已有部分字段通过规则提取完成，现在需要你补充AI生成的字段。
...

{
  ...
  "sub_tables": {
    ...
    "store_product": [
      {"product_name": "菜品/产品名称", "price": 0.00, "description": "产品描述", "currency": "MOP", "is_signature": true, "sort_order": 1}
    ]
  }
}
