-- ============================================================
-- XOOTOUR Macau Database Schema
-- 景点系统 + 5大品类门店系统（美食/酒店/景点/购物/娱乐）
-- ============================================================

CREATE DATABASE IF NOT EXISTS `xootour_aomen` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE `xootour_aomen`;
...
CREATE TABLE `tour_spot` (
    ...
    `city_id` INT NOT NULL DEFAULT 2 COMMENT '城市ID(澳门=2)',
    `cover_img` VARCHAR(500) COMMENT '封面图URL',
    `seo_desc` VARCHAR(500) COMMENT 'SEO描述',
    `base_price` DECIMAL(10,2) DEFAULT 0.00 COMMENT '起步参考价格(MOP)',
...
CREATE TABLE `tour_spot_package` (
    ...
    `currency` VARCHAR(10) DEFAULT 'MOP' COMMENT '币种',
...
CREATE TABLE `store` (
    ...
    `cover_img` VARCHAR(500) COMMENT '封面图URL',
    `seo_desc` VARCHAR(500) COMMENT 'SEO描述',
    `price_range` VARCHAR(100) COMMENT '价格区间(如:人均MOP100-200)',
    `rating` DECIMAL(2,1) DEFAULT 0.0 COMMENT '综合评分',
