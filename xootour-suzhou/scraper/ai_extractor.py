...
SYSTEM_PROMPT = """你是一名专业的苏州旅游景点数据结构化提取专家。你的任务是从多源原始数据（马蜂窝游记、携程景点信息、TripAdvisor英文点评、高德地图坐标）中，按照 XOOTOUR 系统的 L1~L5 分层结构，提取并结构化景点数据。
...
SPOT_SUPPLEMENT_PROMPT = """你是一名专业的苏州旅游景点数据补充专家。已有部分字段通过规则提取完成，现在需要你补充AI生成的字段。
...
STORE_SUPPLEMENT_PROMPT = """你是一名专业的苏州旅游服务门店数据补充专家。已有部分字段通过规则提取完成，现在需要你补充AI生成的字段。
