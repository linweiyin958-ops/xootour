    ...
    def process(row):
        ...
        for sep in [" Chaoyang", " Dongcheng", " Xicheng", " Haidian", " Fengtai",
            ...
            if idx > 0:
                ...
                break
        clean_addr = clean_addr.replace("中国 北京市 ", "").replace("100020 ", "").strip()
    ...
    def nominatim_geocode(name, address):
        ...
        for sep in [" Chaoyang", " Dongcheng", " Xicheng", " Haidian", " Fengtai",
            ...
            if idx > 0:
                clean_addr = clean_addr[:idx]
                break
        clean_addr = clean_addr.replace("中国 北京市 ", "").replace("100020 ", "").replace("100600 ", "").strip()
        q = f"{name}, {clean_addr}, Beijing, China"
        try:
    ...
    def scrape_ta(row):
        ...
        try:
            ...
            if resp.status_code != 200:
                return row["locationId"], row["name"], None
            html = resp.text
            # Extract coordinates - Beijing is around 39.9 N, 116.4 E
