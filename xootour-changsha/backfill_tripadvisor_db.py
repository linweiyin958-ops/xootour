"""Backfill missing fields in tripadvisor-cs.db (tripadvisor_restaurants table).
...
def amap_geocode(address, city="长沙"):
    if not AMAP_API_KEY:
        return None
...
def amap_text_search(name, city="长沙"):
    if not AMAP_API_KEY:
        return None
...
def phase1_geocode(db_path, limit=0, dry_run=False):
    ...
    def process(row):
        ...
        address = row["address"]
        clean_addr = address
        for sep in [" Changsha", " Changzhou"]:
            idx = clean_addr.find(sep)
            if idx > 0:
                clean_addr = clean_addr[:idx]
                break
        clean_addr = clean_addr.replace("中国 长沙市 ", "").strip()
    ...
    def nominatim_geocode(name, address):
        ...
        for sep in [" Changsha"]:
            ...
            if idx > 0:
                ...
                break
        clean_addr = clean_addr.replace("中国 长沙市 ", "").strip()
        q = f"{name}, {clean_addr}, Changsha, China"
        try:
...
def phase4_llm_supplement(db_path, limit=0, dry_run=False, workers=3):
    ...
    system_prompt = "你是一位专业的长沙餐厅数据补充专家。只输出JSON，不要其他内容。"
    ...
    def scrape_ta(row):
        ...
        try:
            ...
            if resp.status_code != 200:
                return row["locationId"], row["name"], None
            html = resp.text
            lat_match = re.search(r'(28\.\d{4,})', html)
            lng_match = re.search(r'(112\.\d{4,}|113\.\d{4,})', html)
...
def main():
    parser = argparse.ArgumentParser(description="Backfill tripadvisor-cs.db missing fields")
    parser.add_argument("--db", default="output/tripadvisor-cs.db", help="Path to SQLite DB")
    ...
    logger.info("=" * 60)
    logger.info(f"Backfill tripadvisor-cs.db | DB: {db_path}")
