...
class AmapWebScraper:
    ...
    def _normalize_poi(self, poi: dict) -> Optional[dict]:
        ...
        if not district and pname:
            ...
            if pname and pname != cityname:
                district = pname + cityname.replace("大理市", "").replace("大理白族自治州", "")
    ...
    def _amap_search(self, keyword: str, city: str = "大理") -> Optional[dict]:
        self._delay()
        try:
            params = {
                "query": keyword,
    ...
    def _nominatim_search(self, name: str, city: str = "Dali") -> Optional[dict]:
        self._delay()
        try:
            params = {
                "q": f"{name}, {city}, China",
    ...
    def search_pois(self, keyword: str, city: str = "大理", max_count: int = 25) -> List[dict]:
