-- ============================================================
-- XOOTOUR Xi'an Database Schema
-- 景点系统 + 5大品类门店系统（美食/酒店/景点/购物/娱乐）
-- ============================================================

CREATE DATABASE IF NOT EXISTS `xootour_xian` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE `xootour_xian`;
...
CREATE TABLE `tour_spot` (
    ...
    `city_id` INT NOT NULL DEFAULT 2 COMMENT '城市ID(西安=2)',
...
(1, '美食', 'Food', 1, '西安美食门店，覆盖陕菜/小吃/面食/泡馍/烤肉/老字号及各地菜系'),
(2, '酒店', 'Hotel', 2, '西安住宿服务，覆盖星级酒店/民宿/青年旅舍/温泉度假'),
(3, '景点', 'Attraction', 3, '西安旅游景点，覆盖5A/4A景区/博物馆/寺庙/历史遗迹/主题乐园'),
(4, '购物', 'Shopping', 4, '西安购物场所，覆盖商圈百货/特色街区/免税店/文创市集'),
(5, '娱乐', 'Entertainment', 5, '西安娱乐场所，覆盖酒吧/KTV/密室/剧本杀/演出/相声/温泉');
...
CREATE TABLE `store` (
    ...
    `cover_img` VARCHAR(500) COMMENT '封面图URL',
    `seo_desc` VARCHAR(500) COMMENT 'SEO描述',
    `price_range` VARCHAR(100) COMMENT '价格区间(如:人均¥50-100)',
    `rating` DECIMAL(2,1) DEFAULT 0.0 COMMENT '综合评分',
