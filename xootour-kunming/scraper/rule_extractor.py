...
from .config import KUNMING_CITY_ID, KUNMING_DISTRICTS
...
class RuleExtractor:
    def extract_spot(
        ...
        if l1["name_zh"] and l1["name_en"]:
            l1["seo_desc"] = f"{l1['name_zh']}({l1['name_en']})北京旅游指南 - 游玩攻略、开放时间、交通路�?
        elif l1["name_zh"]:
            l1["seo_desc"] = f"{l1['name_zh']}北京旅游指南 - 游玩攻略、开放时间、交通路�?
    ...
    def extract_store(
        ...
        if l1["name_zh"] and l1["name_en"]:
            l1["seo_desc"] = f"{l1['name_zh']}({l1['name_en']})-{cat_name}北京旅游服务指南"
        elif l1["name_zh"]:
            l1["seo_desc"] = f"{l1['name_zh']}-{cat_name}北京旅游服务指南"
    ...
    def _merge_dianping_store(self, l1, l2, sub, dianping_data: dict):
        ...
        for review in dianping_data.get("reviews", []):
            ...
            if "英文" in text or "english" in text.lower():
                l2["tags"].append("有英文服�?)
            if "无障�? in text or "wheelchair" in text.lower():
                l2["tags"].append("无障�?)
