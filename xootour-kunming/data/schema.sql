-- ============================================================
-- XOOTOUR Kunming Database Schema
-- 景点系统 + 5大品类门店系统（美食/酒店/景点/购物/娱乐�?-- ============================================================

CREATE DATABASE IF NOT EXISTS `xootour_kunming` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE `xootour_kunming`;
...
CREATE TABLE `tour_spot` (
    ...
    `subtitle` VARCHAR(200) COMMENT '英文全名/副标�?,
    `city_id` INT NOT NULL DEFAULT 1 COMMENT '城市ID(北京=1)',
    `cover_img` VARCHAR(500) COMMENT '封面图URL',
    `seo_desc` VARCHAR(500) COMMENT 'SEO描述',
    `base_price` DECIMAL(10,2) DEFAULT 0.00 COMMENT '起步参考价�?,
...
CREATE TABLE `tour_spot_info` (
    ...
    `district` VARCHAR(100) COMMENT '行政�?,
    ...
    `open_hours` TEXT COMMENT '开放时�?,
    `transportation` TEXT COMMENT '交通方�?,
    `visit_notice` TEXT COMMENT '参观须知',
    `best_visit_season` VARCHAR(100) COMMENT '最佳游玩季�?,
    `photo_spots` JSON COMMENT '打卡机位JSON',
    `crowd_tags` JSON COMMENT '适合人群标签JSON',
    `source_platform` VARCHAR(50) COMMENT '数据源平�?,
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='景点概况�?;
...
CREATE TABLE `tour_spot_route` (
    ...
    `description` TEXT COMMENT '路线描述与避坑指�?,
...
CREATE TABLE `store_category` (
    `id` INT PRIMARY KEY AUTO_INCREMENT,
    `name_zh` VARCHAR(100) NOT NULL COMMENT '品类中文�?,
    `name_en` VARCHAR(100) NOT NULL COMMENT '品类英文�?,
...
(1, '美食', 'Food', 1, '北京美食门店，覆盖京�?烤鸭/涮羊�?小吃/老字号及各地菜系'),
(2, '酒店', 'Hotel', 2, '北京住宿服务，覆盖星级酒�?四合院民�?胡同客栈/青年旅舍'),
(3, '景点', 'Attraction', 3, '北京旅游景点，覆�?A/4A景区/博物�?公园/寺庙/胡同/主题乐园'),
(4, '购物', 'Shopping', 4, '北京购物场所，覆盖商圈百�?特色街区/免税�?文创市集'),
(5, '娱乐', 'Entertainment', 5, '北京娱乐场所，覆盖酒�?KTV/密室/剧本杀/演出/相声/夜店');
...
CREATE TABLE `store` (
    `id` INT PRIMARY KEY AUTO_INCREMENT,
    `name_zh` VARCHAR(200) NOT NULL COMMENT '门店中文�?,
    `name_en` VARCHAR(200) COMMENT '门店英文�?,
    `category_id` INT NOT NULL COMMENT '品类ID',
    `city_id` INT NOT NULL DEFAULT 1 COMMENT '城市ID(北京=1)',
    `cover_img` VARCHAR(500) COMMENT '封面图URL',
    `seo_desc` VARCHAR(500) COMMENT 'SEO描述',
    `price_range` VARCHAR(100) COMMENT '价格区间(�?人均¥100-200)',
    `rating` DECIMAL(2,1) DEFAULT 0.0 COMMENT '综合评分',
    `review_count` INT DEFAULT 0 COMMENT '评论�?,
    `ranking_desc` VARCHAR(200) COMMENT '排名描述',
    `cuisine_tags` JSON COMMENT '菜系标签JSON',
    `michelin_status` INT DEFAULT 0 COMMENT '米其林星�?0=�?',
    `source_platform` VARCHAR(50) COMMENT '数据源平�?,
...
CREATE TABLE `store_info` (
    ...
    `tags` JSON COMMENT '标签JSON(�?外宾友好,可刷外卡,有英文菜�?',
    `category_specific_fields` JSON COMMENT '品类专用字段(按store-prompts.md规范)',
    `subratings` JSON COMMENT '子评分JSON(如食�?服务/性价�?',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='门店概况�?;
