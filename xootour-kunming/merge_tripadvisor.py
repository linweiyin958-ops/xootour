...
TA_DB = os.path.join(os.path.dirname(os.path.abspath(__file__)), "output", "xootour_kunming.db")
...
def init_db():
    logger.info("[Step 1] Initializing Kunming database schema...")
    ...
    for row in ta_rows:
        ...
        try:
            out.execute(
                ...
                "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                (store_id, name, name, DINING_ID, KUNMING_CITY_ID,
                 photos[0].get("image_url") if photos else "",
                 f"{name} Kunming dining guide",
...
def main():
    logger.info("=" * 60)
    logger.info("Kunming TripAdvisor / XOOTOUR Database Merge")
    logger.info("=" * 60)
