...
DB_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "output", "xootour_shenzhen.db")
...
def main():
    ...
    dups = conn.execute(
        "SELECT name_zh, COUNT(*) as cnt, GROUP_CONCAT(id) as ids "
        "FROM store GROUP BY name_zh HAVING cnt > 1 ORDER BY cnt DESC"
    ).fetchall()
    logger.info(f"Found {len(dups)} groups of duplicate store names")
    ...
    for group in dups:
        ...
        rows = conn.execute(
            "SELECT id, data_quality_score, COALESCE(LENGTH(name_en), 0) as en_len "
            "FROM store WHERE id IN ({}) ORDER BY data_quality_score DESC, en_len DESC, id ASC".format(
                ",".join("?" * len(ids))
            ), ids
        ).fetchall()
        ...
        logger.info(f"Group \"{name}\": keep={keep_id}, remove={dupe_ids}")

        for dup_id in dupe_ids:
            merge_info(conn, keep_id, dup_id)

            conn.execute("UPDATE store_banner SET store_id=? WHERE store_id=?", (keep_id, dup_id))
            conn.execute("UPDATE store_facility SET store_id=? WHERE store_id=?", (keep_id, dup_id))
            conn.execute("UPDATE store_product SET store_id=? WHERE store_id=?", (keep_id, dup_id))
    ...
    logger.info(f"Removed {total_removed} duplicate stores")
