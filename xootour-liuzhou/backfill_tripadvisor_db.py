...
def amap_geocode(address, city="柳州"):
    if not AMAP_API_KEY:
        return None
...
def amap_text_search(name, city="柳州"):
    if not AMAP_API_KEY:
        return None
    ...
    def process(row):
        ...
        for sep in [" Chaoyang", " Dongcheng", " Xicheng", " Haidian", " Fengtai",
            ...
            if idx > 0:
                ...
                break
        clean_addr = clean_addr.replace("中国 广西壮族自治区 ", "").strip()
    ...
    def nominatim_geocode(name, address):
        ...
        for sep in [" Chaoyang", " Dongcheng", " Xicheng", " Haidian", " Fengtai",
            ...
            if idx > 0:
                ...
                break
        clean_addr = clean_addr.replace("中国 广西壮族自治区 ", "").strip()
        q = f"{name}, {clean_addr}, Liuzhou, China"
        try:
...
def phase4_llm_supplement(db_path, limit=0, dry_run=False, workers=3):
    ...
    system_prompt = "你是一位专业的柳州餐厅数据补充专家。只输出JSON，不要其他内容。"
    ...
    def scrape_ta(row):
        ...
        try:
            ...
            if resp.status_code != 200:
                return row["locationId"], row["name"], None
            html = resp.text
            # Extract coordinates - Liuzhou is around 24.3 N, 109.4 E
