...
def main():
    ...
    logger.info(f"Found {len(dups)} groups of duplicate amap_ids")
    ...
    for group in dups:
        ...
        rows = conn.execute(
            "SELECT si.store_id, s.data_quality_score FROM store_info si "
            "JOIN store s ON s.id = si.store_id "
            "WHERE si.store_id IN ({}) ORDER BY s.data_quality_score DESC, si.store_id ASC".format(
                ",".join("?" * len(ids))), ids
        ).fetchall()

        keep_id = rows[0]["store_id"]
        dupe_ids = [r["store_id"] for r in rows[1:]]
    ...
    logger.info(f"Removed {total_removed} duplicate amap_id stores")
