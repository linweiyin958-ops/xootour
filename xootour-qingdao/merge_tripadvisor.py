...
TA_DB = os.path.join(os.path.dirname(os.path.abspath(__file__)), "output", "xootour_qingdao.db")
...
def init_db():
    logger.info("[Step 1] Initializing Qingdao database schema...")
    ...
    for row in ta_rows:
        ...
        try:
            out.execute(
                ...
                "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                (store_id, name, name, DINING_ID, QINGDAO_CITY_ID,
                 photos[0].get("image_url") if photos else "",
                 f"{name} Qingdao dining guide",
...
def main():
    logger.info("=" * 60)
    logger.info("Qingdao TripAdvisor / XOOTOUR Database Merge")
    logger.info("=" * 60)
