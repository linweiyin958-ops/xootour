"""Backfill missing fields in tripadvisor-sanya.db (tripadvisor_restaurants table).
...
def amap_geocode(address, city="三亚"):
    if not AMAP_API_KEY:
        return None
...
def amap_text_search(name, city="三亚"):
    if not AMAP_API_KEY:
        return None
...
def phase1_geocode(db_path, limit=0, dry_run=False):
    ...
    def process(row):
        ...
        address = row["address"]
        clean_addr = address
        for sep in [" Sanya", " Jiyang", " Tianya", " Haitang", " Yazhou",
                      " Dadonghai", " Yalong Bay", " Sanya Bay"]:
            idx = clean_addr.find(sep)
            if idx > 0:
                clean_addr = clean_addr[:idx]
                break
        clean_addr = clean_addr.replace("Hainan ", "").replace("Sanya ", "").strip()
    ...
    def nominatim_geocode(name, address):
        ...
        for sep in [" Sanya", " Jiyang", " Tianya", " Haitang", " Yazhou"]:
            ...
            if idx > 0:
                ...
                break
        clean_addr = clean_addr.replace("Hainan ", "").replace("Sanya ", "").strip()
        q = f"{name}, {clean_addr}, Sanya, China"
        try:
...
def phase4_llm_supplement(db_path, limit=0, dry_run=False, workers=3):
    ...
    def build_prompt(row):
        ...
        if not row.get("description"):
            needs.append("description (简介/特色介绍, 80-150字中文)")
        if not row.get("features"):
            needs.append("features (设施特点, 逗号分隔的中文列表, 如：免费WiFi,可预订,有包间等)")
    ...
    system_prompt = "你是一位专业的三亚餐厅数据补充专家。只输出JSON，不要其他内容。"
    ...
    def scrape_ta(row):
        ...
        try:
            ...
            if resp.status_code != 200:
                return row["locationId"], row["name"], None
            html = resp.text
            lat_match = re.search(r'(1[89]\.\d{4,})', html)
            lng_match = re.search(r'(10[89]\.\d{4,})', html)
...
def main():
    parser = argparse.ArgumentParser(description="Backfill tripadvisor-sanya.db missing fields")
    parser.add_argument("--db", default="output/tripadvisor-sanya.db", help="Path to SQLite DB")
    ...
    logger.info("=" * 60)
    logger.info(f"Backfill tripadvisor-sanya.db | DB: {db_path}")
