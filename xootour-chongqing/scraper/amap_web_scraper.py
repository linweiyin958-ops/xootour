...
class AmapWebScraper:
    ...
    def _normalize_poi(self, poi: dict) -> Optional[dict]:
        ...
        if not district and pname:
            ...
            if pname and pname != cityname:
                district = pname + cityname.replace("重庆市", "")
    ...
    def _amap_search(self, keyword: str, city: str = "重庆") -> Optional[dict]:
        self._delay()
        try:
            params = {
                "query": keyword,
    ...
    def search_pois(self, keyword: str, city: str = "重庆", max_count: int = 25) -> List[dict]:
