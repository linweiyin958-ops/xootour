...
TA_DB = os.path.join(os.path.dirname(os.path.abspath(__file__)), "output", "xootour_chongqing.db")
XO_DB = os.path.join(os.path.dirname(os.path.abspath(__file__)), "output", "xootour_chongqing.db")
...
def init_db():
    logger.info("[Step 1] Initializing Beijing database schema...")
...
def step2_migrate_tripadvisor():
    """Migrate tripadvisor_restaurants from source DB."""
    ...
    for row in ta_rows:
        ...
        try:
            out.execute(
                ...
                "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                (store_id, name, name, DINING_ID, CHONGQING_CITY_ID,
                 photos[0].get("image_url") if photos else "",
                 f"{name} Beijing dining guide",
    ...
    out.close()
    conn.close()
    conn_src.close() if 'conn_src' in dir() else None
...
def main():
    logger.info("=" * 60)
    logger.info("Beijing TripAdvisor / XOOTOUR Database Merge")
    logger.info("=" * 60)
