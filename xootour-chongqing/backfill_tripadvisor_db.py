...
def amap_geocode(address, city="重庆"):
    if not AMAP_API_KEY:
        return None
...
def amap_text_search(name, city="重庆"):
    if not AMAP_API_KEY:
        return None
    ...
    def process(row):
        ...
        for sep in [" Chaoyang", " Dongcheng", " Xicheng", " Haidian", " Fengtai",
            ...
            if idx > 0:
                ...
                break
        clean_addr = clean_addr.replace("中国 重庆市 ", "").strip()
...
def phase4_llm_supplement(db_path, limit=0, dry_run=False, workers=3):
    ...
    system_prompt = "你是一位专业的重庆餐厅数据补充专家。只输出JSON，不要其他内容。"
